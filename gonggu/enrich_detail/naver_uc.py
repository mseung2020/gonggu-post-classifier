"""네이버 전용 렌더링 엔진 — undetected_chromedriver(uc) + 전용 프로필 (macOS 이식).
...
걷어내고 macOS 기준으로 이식했다. DETAIL_NAVER_ENGINE=uc일 때 네이버 계열 URL만 이 경로를
탄다(다른 몰은 기존 Playwright 그대로 — fetchpage.py 참고).

uc/selenium은 동기(sync)라 드라이버 1개를 락으로 직렬화한다 — 네이버 항목은 워커가 몇 개든
한 번에 하나씩 처리된다(어차피 같은 도메인에 몰리는 걸 막아야 하니 페이싱 효과도 겸함).

로그인월/챌린지가 뜨면 실제 창에서 사람이 직접 통과하도록 안내하고 기다린다 — 한 번
통과하면 프로필에 쿠키가 저장되어 이후 실행은 자동으로 지나간다(개발자 실증 동작).

환경변수:
    DETAIL_NAVER_ENGINE=uc   이 엔진 활성화(기본은 비활성 — Playwright)
