...
dedup(이미 분류된 key인지)은 CLASSIFIED_DIR 전체를 매번 다시 읽지 않고
common.load_classify_done_keys()의 작은 인덱스로 확인한다(2026-08-18, 문제 1/9 — 02_classified가
실측 223MB+, 하루 5~12MB씩 자라는데 이 파일을 classify.py/classify_yt_ppl.py가 매일 각자
전체 재파싱하던 걸 없앴다). classify_yt_ppl.py와 이 인덱스를 공유한다 — 두 스크립트가 다루는
key 공간(ig/yt 키워드매칭 vs yt PPL)은 fetch 단계 SQL에서부터 서로 배타적이라 안전하다.

⚠ 이 dedup은 key(post_id/video_id) 단위다 — 한 번 성공적으로 분류된 포스트는 이후 hifen 쪽
캡션이 수정돼도 재분류하지 않는다(의도된 정책, 2026-08-18 확인 — 실제로 게시 후 캡션이
수정되는 경우가 없다고 판단해 재검토 로직을 두지 않기로 함).
...
from gonggu.common import (CLASSIFIED_DIR, DEEPSEEK_KEY, RAW_DIR, append_jsonl, call_llm,
...
def _persist(r):
    """CLASSIFIED_DIR에 결과를 남기고, 성공한 경우에만 공유 done-키 인덱스에도 기록한다
    (실패/에러는 인덱스에 안 남아야 다음 실행에서 다시 todo에 잡혀 재시도된다)."""
...
def main():
    ...
    already_done = sum(1 for p in posts if _key(p) in done_keys)
    print(f'전체 {len(posts)} | 완료 {already_done} | 이번 실행 {scope}{len(todo)}건 (동시 {concurrency})')

    # 결과 1건 = 그 날짜 파일 끝에 한 줄 추가(append)만 한다 — 건수가 쌓여도 저장 비용이
    # 늘지 않는다(2026-07-27 실측/전환, 자세한 사연은 common.append_jsonl 참고).
    counters = run_llm_batch(todo, classify_one, _persist,
                             concurrency=concurrency, ok_start=len(done_keys))

    total_done = len(done_keys) + counters['ok'] + counters['err']
    print(f'총 {total_done}건(성공 {len(done_keys) + counters["ok"]} / 실패 {counters["err"]}) '
