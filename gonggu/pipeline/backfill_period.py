...
사용법:
    ...
    PERIOD_INPOCK_CONCURRENCY=150 python3 -m gonggu.backfill_period  # Tier0(인포크, 브라우저 무관) 동시성 — daily 기본값
    BACKFILL_PERIOD_CONCURRENCY=14 MAX_BROWSERS=14 python3 -m gonggu.backfill_period  # Tier1(몰 크롤) — daily 기본값
    # ⚠ Tier1 동시성은 MAX_BROWSERS와 짝으로 맞출 것(워커 ≈ 브라우저×1.14) — 이유는 daily.py의
    # backfill_period 항목 주석 참고(2026-08-20 재조정: 8/자동계산10 -> 14/14, 실측 48건/분 이상 기대)
"""
import datetime
import hashlib
import os
...
def _merge_sources(prev, new_sig):
    """이전 체크포인트 기록(prev, 없으면 None)의 sources와 이번에 읽은 지문(new_sig)을 합집합.

    ⚠ 2026-08-20 회귀 버그 — 여기 있던 `prev.get('sources')`를 `if prev else` 가드 없이 불러서,
    prev가 None인 가장 흔한 경우(체크포인트에 이력이 없는 첫 시도 항목)마다
    `AttributeError: 'NoneType' object has no attribute 'get'`로 죽었다. 바로 위 attempts 계산은
    가드가 있었는데 이 줄만 빠뜨렸다 — 실전에서 11,898건 중 거의 전부가 이 경로를 타면서 발견됐다
    (크롤 풀이 항목당 예외를 삼키고 건너뛰어서 프로세스는 안 죽었지만, 기간을 하나도 못 찾았다).
    합집합인 이유는 _should_skip 참고 — 예전에 읽었던 소스를 잊으면 무한 재시도가 된다."""
    prev_sources = set(prev.get('sources') or []) if prev else set()
    return sorted(prev_sources | new_sig)
...
def main():
    ...
    def _finish(ctx, code, key, row_id, start, end, note, tier):
        ...
        sources = _merge_sources(prev, sigs.get(key, set()))
