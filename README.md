...
`resolve_links.py`는 실제 크롤링(안티봇 회피 대기 포함)이라 느립니다 — 아직 안 돌렸거나
건너뛰면 `load.py`는 `transform.py`가 만든 원본 후보 목록(세미콜론으로 이어붙인 상태)을
그대로 씁니다. **`run_all.py`(자동 반복 루프)에는 아직 연결 안 해뒀습니다** — 매 청크마다
크롤링이 끼어들면 라운드 하나가 훨씬 오래 걸리게 되니, 먼저 `resolve_links.py`를 따로
충분히 검증한 뒤에 넣을지 결정할 것.
...
번갈아 classify → transform → load까지 반복 실행합니다. 한쪽 플랫폼이 다 끝나면 자동으로
